...
async def connect(
    ...
    if not has_credentials and not has_cookie and not has_azure and not has_token:
        raise HTTPException(
            status_code=422,
            detail="Provide one of: service_account_token, (username + password), cookie_header, or azure_scope.",
        )
    ...
    if has_token:
        ...
        try:
            ...
            raise HTTPException(
                status_code=401,
                detail=f"Service account token auth failed: {exc}. Check the token has at least Viewer permissions.",
            ) from exc
