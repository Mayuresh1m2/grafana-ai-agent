...
class AgentQueryRequest(BaseModel):
    ...
    session_id: str | None = Field(default=None, description="Active Grafana session ID.")
    namespace: str | None = Field(default=None, description="Kubernetes namespace in scope.")
    environment: str | None = Field(default=None, description="Environment (prod, staging, …).")
    services: list[str] = Field(default_factory=list, description="Services in scope.")
    active_alerts: str | None = Field(default=None, description="Pre-fetched alert summary.")
    grafana_url: str | None = Field(default=None, description="Grafana base URL.")
...
class GrafanaConnectRequest(BaseModel):
    ...
    service_account_token: str | None = Field(
        default=None,
        description=(
            "Grafana service account token (glsa_…). Also used by the MCP server. "
            "Create in Grafana → Administration → Service Accounts."
        ),
    )
